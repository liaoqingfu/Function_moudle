...
unsigned int ota_down_states = 0;			//下载状态 0为TF card下载路径，1为其他下载路径
...
int Assignment_value(zx_Json *res_root,VERSION_UPGRADE *upgrade)
{
	...
	{
		dzlog_error("upgrade->device_type is NULL");
		return -1;
	}
	...
	{
		dzlog_error("upgrade->introduction is NULL");
		return -1;
	}
	...
	{
		dzlog_error("upgrade->rom_version_name is NULL");
		return -1;
	}
	
	upgrade_data = cJSON_GetObjectItem(upgrade_data, "full_package");

	if( !upgrade_data || strcmp(cJSON_PrintUnformatted(upgrade_data),"null") == 0  )
	{
	...
	if(!s_result || strcmp(s_result,"null") == 0 )
	{		
	...
	if(!upgrade->down_url )
	{
		dzlog_error("upgrade->down_url is NULL");
		return -1;
	}
	...
	if(!s_result )
	{
		dzlog_error("upgrade->file_name is NULL");
		return -1;
	}
...
int zx_get_upgrade_file_md5(const char *file_path, char *md5_str)  
{  
	int i = 0;  
    FILE* fd = NULL;
    int ret = -1;  
	...
	if( !fd )
	{
		...
		printf_hex(file_path,strlen(file_path));
        return -1;
	}

    zx_MD5Init(&md5);  // init md5
    while (1)  
    {  
...
VERSION_UPGRADE *get_device_type_info(VERSION_UPGRADE *pPre,const char *device_type)
{
	...
	if(!pCur)
	{
		dzlog_error("no could device_type: %s", device_type);
		return NULL;
	}
...
int Upgrade_Info_ListPrint(VERSION_UPGRADE * pPre, const char *device_type)
{
	...
	{
		while(pCur->device_type && (strcmp(pCur->device_type,device_type) != 0)) //查找需要打印的设备版本信息
		{
			pCur = pCur->next;
	...
	) 
{
	...
	dzlog_info(" get all upgrade version info start, sn=[%s]" , sn);
	...
	while(1)
	{
		...
		if(!s_result )
		{
			dzlog_error("json data is ERROR ");
			break;
		}
...
int zx_md5_fwrite_file(char *file_path, char *md5_buf)
{
	FILE *md5_fp = NULL;
	...
	md5_len = strlen(md5_buf);

	if( 0 == file_path_len || 0 == md5_len )
	{
		dzlog_error("file path len or md5 len is zero");
		return ret;
	}

#if OTA_DEBUG_LOG
	dzlog_info("【 MD5 LEN = %d 】",md5_len);
#endif

	md5_file_path = (char *)malloc(file_path_len + 6);

	if(!md5_file_path)
	{
		dzlog_error("md5 file path malloc error");
		return ret;
	}

	memset(md5_file_path,0,file_path_len + 6);

	sprintf(md5_file_path,"%s.md5",file_path);

	file_path_len = strlen(md5_file_path);
	
#if OTA_DEBUG_LOG
	dzlog_info("md5_file_path = %s",md5_file_path);
#endif

	md5_fp = fopen(md5_file_path,"w");

	if(!md5_fp)
	{
		dzlog_error("fopen error file path : %s",md5_file_path);
		printf_hex(md5_file_path,file_path_len);
		goto MD5_FILE_PATH_FREE;
		
	}

	if(fwrite(md5_buf,1,md5_len,md5_fp) != md5_len)
	{
...
int zx_md5_fread_file(char *file_path, char *md5_buf)
{
	FILE *md5_fp = NULL;
	int ret = -1;
	int file_path_len = -1;
	...
	if( 0 == file_path_len )
	{
		dzlog_error("file path len is zero");
		return ret;
	}

	md5_fp = fopen(file_path,"r");

	if(!md5_fp)
	{
		dzlog_error("fopen error file path : %s",file_path);
		printf_hex(file_path,file_path_len);
		return ret;
		
	}

	if(fread(md5_buf,1,MD5_OTA_SIZE*2,md5_fp) != MD5_OTA_SIZE*2)
	{
		dzlog_error("【fread error errno:%d===>[%s]】",errno,strerror(errno));
		goto MD5_FP_CLOSE;
...
#if OTA_DEBUG_LOG
	dzlog_info("【fread md5 = %s】",md5_buf);
#endif
...
int zx_dnload_upgrade_version(
	...
	char * default_filename             //自定义文件名,如果不写, 默认为服务器上的文件名
)
{
	...
	path_len = strlen(down_path);

    if (access(down_path,F_OK) != 0)   //判断目录是否正确的
    {  
        dzlog_error("filename_path error:%s",down_path);
		printf_hex(down_path,strlen(down_path));
        return -1;  
    }  

	if(strcmp(down_path,VERSION_TF_PATH) == 0)
	{
		ota_down_states = 0;			//TF卡下载路径
		
	}else{

		ota_down_states = 1;			//其他下载路径
	}

	if( !default_filename  ||  strlen( default_filename ) <= 0  )
	{
		filename_len = strlen(upgrade->file_name);
		num = path_len + filename_len + 6;
	...
	{
		num = path_len + strlen( default_filename ) + 6 ; 
		filename_path = (char *)malloc(num);
		if( !filename_path )
		{
			dzlog_error(" out of memory for filename_path. ");
	...
	if(dev_filesize == 0)
	{
		dzlog_error("get file size error");
		goto END;
	}
	...
	while(dev_filesize != upgrade->file_size)
	{
...
#endif	
		dzlog_info("down count number******: %d",++i);
		...
		if(dev_filesize == 0)
		{
			dzlog_error("get file size is : %ld",dev_filesize);
		}
		
		if(i >= 3)
		{
			ret = -1;
			dzlog_error("******Compare file size differently  ******");
			goto END;
		}
	...
	ret = zx_get_upgrade_file_md5(filename_path,md5_str); //下载文件生成md5
	...
	//将md5写入文件
	ret = zx_md5_fwrite_file(filename_path,md5_str);

	if(ret != 0)
	{
		dzlog_error("md5 fwrite file error");
		ret = -1;
		goto END;
	}
	
#if 0
	char ota_md5_file[128] = {0};
	char read_md5_str[ MD5_OTA_SIZE * 2 + 2 ]={0};
	sprintf(ota_md5_file,"%s.md5",filename_path);
	
	ret = zx_md5_fread_file(ota_md5_file,read_md5_str);

	if(ret != 0)
	{
		dzlog_error("md5 fread file error");
		ret = -1;
		goto END;
	}

	dzlog_info("【 fread md5 file = %s】",read_md5_str);
#endif
...
// add by yuxw
int check_upgrade_file_with_md5( char * file_path )
{
	int ret = -1;
	char file_md5[ 64 ] = { 0 };
	
	//char file_path_md5 [ 128  + 34 ] = { 0 };
	int file_path_md5_len = -1;
	
	char * file_path_md5 = NULL;
	...
	file_path_md5_len = strlen( file_path );

	if( 0 == file_path_md5_len )
	{
		dzlog_error("file path len is zero");
		return ret;
	}
	
	file_path_md5 = (char *)malloc( file_path_md5_len + 8 ); 
	
	if( !file_path_md5 )
	{
		dzlog_error("out of memory of get file_path_md5. fail.");
		goto END;		
	}

	memset(file_path_md5,0,file_path_md5_len + 8 );
	
	// read file and compute md5
	ret = zx_get_upgrade_file_md5( file_path, file_md5 ); // file's md5
	
	if(ret == -1)
	{
		dzlog_error("****** fail to compute file md5.******");
		goto END;	
	}
	
	
	// read the file md5 by file <file_path>.md5

	snprintf( file_path_md5, file_path_md5_len  + 6 ,"%s.md5", file_path );
	
	ret = zx_md5_fread_file(file_path_md5, read_md5_str );

	if(ret != 0 )
	{
		dzlog_error("fail to read md5 file:%s.", file_path_md5 );
		ret = -1;
		goto END;
	}

	// compare md5
	if( strcmp( read_md5_str, file_md5 ) )
	{
		dzlog_error("compare md5 fail. file_md5:%s, read_md5 from file<%s>:%s.", file_md5, file_path_md5, read_md5_str );
		ret = -1;
		goto END;
	...
	{
		ret = 0;
		dzlog_info("【 The upgrade file:%s, md5 check OK.】", file_path );
	}
