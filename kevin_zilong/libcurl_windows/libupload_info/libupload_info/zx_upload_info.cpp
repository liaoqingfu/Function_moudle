...
zx_Error zx_device_Info_base(
	char *as_host,		//域名	"https://security-app.eufylife.com"
	int device_channel,
	char *device_model,
	char *device_name,
	char *device_sn,
...
)
{
	...
	n = strlen(main_sw_version);
	
	version = (char *)malloc(n + 1);
	...
	memset(version,0,n + 1);

	memcpy(version,main_sw_version,n);
	//printf("【strlen(version) = %d\n】",strlen(version));
	if(version[n - 1] == '\n')
	{
		version[n - 1] = 0;
	}
	...
	if (http_url)
	{
		...
		if (param_root)
		{
			...
			cJSON_AddStringToObject(param_root,"main_hw_version",main_hw_version);
			cJSON_AddStringToObject(param_root,"main_sw_version",version);
			cJSON_AddStringToObject(param_root,"sec_hw_version",sec_hw_version);
			cJSON_AddStringToObject(param_root,"sec_sw_version",sec_sw_version);
...
int zx_delect_last_r_or_n(char *data)
{
	int ret = -1, n = 0;
	char *get_data = NULL;
	...
	if(!data)
	{
		printf("The function paremeter is NULL");
		return -1;
	}

	n = strlen(data);

	get_data = (char *)malloc(n + 1);
	...
	memset(get_data,0,n + 1);
	memcpy(get_data,data,n);

	if(get_data[n-1] == '\r' || get_data[n-1] == '\n')
	{
		get_data[n-1] = 0;
	}
	memset(data,0,n );
	memcpy(data,get_data,strlen(get_data));
