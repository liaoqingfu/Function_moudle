...
./a.out error
export LD_LIBRARY_PATH=. (第一种解决办法)
sudo cp libfunc.so /usr/lib (第二种解决办法)

静态库的创建:
gcc func.c -c  生成 func.o
ar rcs libfunc.a func.o
如果静态库和动态库重名，优先链接动态库
gcc main.c -lfunc -static -L. 指定使用静态库

如果静态库和动态库重名，并且要同时用到静态库和动态库
...
动态库的使用：
	...
	4. 使用环境变量： 
		...
		例如： export LD_LIBRARY_PATH=/home/zilong/test/lib:$LD_LIBRARY_PATH  //临时设置，等号两边不能有空格
